...
5. 发布后核对 registry 版本、integrity、provenance 和隔离安装；
   再创建或补全 GitHub Release `v2.0.1`。已发布的 tgz 和标签不可替换。
...
- [ ] `node tools/version.mjs --check`、完整 Go/Lua 和工具测试通过。
- [ ] `v2.0.1` 指向已合并主分支的验收提交，工作树干净。
- [ ] Windows amd64 必需 CI、发行组装、运行 smoke、封存摘要全部通过。
- [ ] npm Trusted Publisher 绑定本仓库和 `toolkit-release.yml`。
- [ ] registry 状态确认 2.0.1 可发布，且不存在不同字节冲突。
- [ ] 同一 tgz 完成 OIDC 发布、registry 回读和 GitHub Release。
