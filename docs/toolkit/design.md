...
- 已发布的修复版为 `2.0.1`，Git tag 为 `v2.0.1`，npm 包为 `lycheedev@2.0.1`；当前候选为 `2.0.3`。后续版本须单独通过发行门禁；不得改写既有标签或包。
...
报告与后续输入就绪共用一张 QR：光学封装 `lycheedev.receipt.v1` 保留原回执，
另带 `[sessionNonce, sequence, runtimeEpoch]` 就绪元组。宿主还原为两份独立信号，
就绪序号必须更新、nonce 必须一致，每次观察仍要求新到达帧；该封装不写入报告
存储，不改变报告摘要。焦点、战斗、离开世界使整张码失效，恢复时重新观察就绪。
独立 `ready` 必须携带自己的 nonce，发送端保留角色身份以支持无输入的 bind；
已建立会话的精简回执只能由已证明的基线补齐。双端须配套升级，旧宿主不理解新封装。
...
2.0.2 候选插件验收范围为 Retail 120100、Classic 50504、Titan 38002；Forever 16001（camelot 混合内核，数据发布于 wow_classic_beta 清单槽位）已通过完整真机链验收（connect→load→run→ack→hide，2026-09-25），实际 Build 在每次测试记录。源码和静态数据可保留更广的查询能力，但不会扩大插件安装范围。
