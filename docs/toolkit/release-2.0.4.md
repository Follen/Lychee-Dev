# Lychee Dev Toolkit 2.0.4：Windows CI 与 npm 发布合同

状态：2.0.4 候选，尚未发布。日期：2026-09-26。2.0.1 已发布，其标签、
npm 包和发行字节不可改写；历史发布与恢复记录见
[2.0.1 合同](release-2.0.1.md)。本文件只约束 2.0.4 候选及其最终发布。

## 本次修复

- skill 将二维码、已连接和 loaded 明确作为中间状态；探针读取报告后继续同 operation ACK、最终 hide，并验证清除；连接和 reload 使用各自完成条件。
- Windows skill launcher 直接调用 npm 包的 JavaScript 入口，保留参数及退出码，不通过 cmd shell。
- 多 Interface TOC 的静态矩阵检查采用精确 token 匹配。
- listfile 查询支持有界 --max-bytes（默认 256 MiB，最大 512 MiB）及 120 秒下载期限，保留预算错误分类；实际 153 MB 社区 listfile 四种查询通过。
- 英文默认与简体中文 README 增加功能、开发者、Agent、安装及证据边界说明。
- 55 项迁移清单、76 次 CLI 调用的结果见 [业务回归报告](business-regression-2026-09-26.md)。17 次语义查询缺密钥阻塞；新 Agent 自动触发与本轮真机二维码闭环保持 not_run。
...
必须可用。正式 2.0.4 使用 `latest`；若先发行 `2.0.4-rc.N`，只能进入
`next`，不得用正式版本号试装。
...
- [ ] registry 中不存在冲突的 `lycheedev@2.0.4`。
- [ ] OIDC publish、registry 回读和 GitHub Release 使用同一 sealed tgz 完成。

在以上项目完成前，2.0.4 只能称为候选，不得称为已上线或可发布。
