# Lychee Dev Toolkit 2.0.5：Windows CI 与 npm 发布合同

状态：2.0.5 候选，尚未发布。日期：2026-09-27。2.0.1 已发布，其标签、
npm 包和发行字节不可改写；历史发布与恢复记录见
[2.0.1 合同](release-2.0.1.md)。本文件只约束 2.0.5 候选及其最终发布。

## 本次修复

- 无报告 abandon 保留完整派发观察，修复 abandoning 中断恢复和终态重复 abandon/resume；执行结果仍未知，不冒充 ACK 或完整成功。
- describe、skill 与设计/回归合同同步完整派发后的显式放弃条件，警告不再假称一定存在已归档报告。
- 项目自有代码与 npm LICENSE 统一 MIT，README 移除旧项目介绍及双许可说明；真实第三方许可正文保持不变。
- 2026-09-27 经用户明确决定，正式服旧任务成功 abandon，随后的同 ID resume 幂等通过。新探针尚未加载：现有游戏运行态 2.0.3 与本地构建 2.0.4 不匹配，未绕过校验。此观察不代表本候选新探针真机闭环通过。
...
必须可用。正式 2.0.5 使用 `latest`；若先发行 `2.0.5-rc.N`，只能进入
`next`，不得用正式版本号试装。
...
- [ ] registry 中不存在冲突的 `lycheedev@2.0.5`。
- [ ] OIDC publish、registry 回读和 GitHub Release 使用同一 sealed tgz 完成。

在以上项目完成前，2.0.5 只能称为候选，不得称为已上线或可发布。
