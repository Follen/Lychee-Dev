# Lychee Dev Toolkit 2.0.3：Windows CI 与 npm 发布合同

状态：2.0.3 候选，尚未发布。日期：2026-09-26。2.0.1 已发布，其标签、
npm 包和发行字节不可改写；历史发布与恢复记录见
[2.0.1 合同](release-2.0.1.md)。本文件只约束 2.0.3 候选及其最终发布。

## 本次修复

- v2.0.2 在源码归档门禁失败，未发布 npm 包；保留该标签。
- 修复旧 Mainline TOC 检查，源码预检在重型 CI 前执行。
- 两组独立 live 生命周期场景并行，CI 限制为 4；不减少断言或落盘。
- BLTE 缺钥和格式错误按能力/输入错误返回，避免泛化为外部失败。
...
必须可用。正式 2.0.3 使用 `latest`；若先发行 `2.0.3-rc.N`，只能进入
`next`，不得用正式版本号试装。
...
- [ ] registry 中不存在冲突的 `lycheedev@2.0.3`。
- [ ] OIDC publish、registry 回读和 GitHub Release 使用同一 sealed tgz 完成。

在以上项目完成前，2.0.3 只能称为候选，不得称为已上线或可发布。
