...
- 2026-09-27 桥接审计修复候选 2.0.6：B01 受管 from/to reload 与公开请求幂等；
  ...
  B05 保持显示尺寸的分数像素解码；B06 自动有界 ROI 与实际坐标；B07 修正未知
  输入解释。D1 新增 live finish，清屏证据持久化及成功重试零输入；D2 恢复性能
  调查方法。定向与全仓库 Go/Lua 回归、build/vet、51 项 Node 测试、80 commands /
  192 references skill 合同通过；发布门禁执行记录以候选合同和 Actions 为准。
  尚未以本候选完成新的真实游戏测试，不将模拟缩放/事件测试写成真机结果。
...
- 2026-09-26 回执传输精简（深协议优化）与二维码可读性：Classic 真机截帧确认
  ...
  4. **卡片尺寸的真正约束**（真机截帧度量，非推算）：卡片模块被"符号高度
     预算"钉死，而不是被目标值决定。并排布局下卡片宽 = 两符号之和，模块只有
     ~2.5 物理像素；改成上下堆叠后宽只剩一个符号，但高变成两符号之和，模块
     3 UI 仍然封顶（宽允许 6、高只允许 3）。实测一张 65 模块回执：
     并排 170×324 UI（模块 2，难采样）、堆叠成对 219×426 UI（模块 3）、
     单符号 219×219 UI（模块 6）。也就是说"成对显示"本身是尺寸的主要成本，
     而协议瘦身把模块数从 77 降到 65 才是无代价的收益。单符号布局已尝试并
     回退：成对显示承载了多个套件断言的序列时序，改动风险高，留作独立议题。
...
## 2026-09-26 发布门禁修复与 CI 耗时

- `v2.0.2` 发布任务 36233362005：所有 CI 门禁通过，但源码归档仍要求已移除的 `addon/Lychee Dev_Mainline.toc`，组装失败，npm 未发布；保留原标签，后继候选为 2.0.3。
- 源码归档与轻量预检共用必需路径，改为 `addon/Lychee Dev.toc`。预检读取 Git 提交中的文件，发布 identity 通过后才启动完整 CI/CGO；归档本身仍校验文件并离线重建比较。
- 本机 CPU profile：生命周期测试主要消耗在 Windows 文件系统调用，含路径规范化与 FlushFileBuffers；不以禁用落盘换取速度。
- 两组完全独立的生命周期子场景并行，CI `-parallel=4`。同机 full live 包从 127.51s 降至 89.47s；其中两组从 34.57s/19.36s 降至约 14s/7s（并行父测试的 elapsed 不含等待，使用 run/pass 时间差）。这是本机数据，托管 Windows race 提速幅度尚待 CI。
- required job 名称、全套测试、race、Windows amd64 产品范围、源码重建比较、OIDC 和 sealed artifact 门禁均保留。main/tag 两次完整验证暂时保留，尚未实现按精确提交复用 CI 证据。
