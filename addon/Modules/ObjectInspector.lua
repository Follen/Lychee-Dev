...
-- Read-only object inspection over _G paths and captured values. Every read of
-- an inspected object is bounded and passive: tables are read with rawget,
-- non-tables are indexed under pcall, and nothing is ever written back to an
-- inspected or Blizzard-owned object.
local inspector = {}
...
local MAX_TEXT_CHUNK_BYTES = 44000
...
local function IsCombatBlocked()
    return ns.Safety.IsCombatBlocked()
end
...
-- Path grammar: identifier | .identifier | [int] | ["str"] | ['str'] over _G.
...
local function SafeMethod(value, methodName)
    ...
    if not methodSucceeded or type(method) ~= "function" then
        return nil
    end
...
-- One inspection = label + type + bounded preview + bounded text snapshot with
-- its incremental stream + a live value tree. Frame targets gain the UI object
-- overview/children/regions nodes through the tree builder.
local function BuildInspection(value, label)
    if IsSecret(value) then
        return nil, ns.L.SECRET_VALUE_BLOCKED
    end
    local isFrame = IsScriptRegion(value)
    local textStream = ns.Serializer.CreateStream(value)
    local serialized = textStream:ReadChunk(MAX_TEXT_CHUNK_BYTES)
    return {
        label = label,
        ...
        tree = ns.Inspector.CreateLiveTree({ n = 1, value }),
    }
end
...
-- Function targets must end in a string key: the key names the hooked method
-- and the rest of the path resolves to its owner table.
function inspector.ResolveFunctionTarget(path)
    local tokens, errorMessage = ParsePath(path)
    if not tokens then
        return false, nil, nil, errorMessage
    end
    ...
    if not succeeded then
        return false, nil, nil, resolveError
    end
...
function inspector.CaptureMouseFocus()
    ...
    local focus = ns.Compat.GetMouseFocus()
    if not focus then
...
function inspector.GetMouseFocusLabel()
    if IsCombatBlocked() then
        return false, nil, nil, ns.L.COMBAT_BLOCKED
    end
    local focus = ns.Compat.GetMouseFocus()
    if not focus then
        return false, nil, nil, ns.L.NO_MOUSE_FOCUS
    end
    return true,
...
-- Case-insensitive key substring search ranked exact -> prefix -> contains,
-- each bucket key-sorted. Bounds: 200 results / 10000 scans / 2000 visited
-- nodes / depth 6. Secret keys and values are skipped; the walk descends
-- tables, GetChildren/GetRegions results and metatable.__index tables.
function inspector.SearchValue(value, query, limit, maxDepth)
    ...
    if query == "" then
        return false, nil, ns.L.SEARCH_TERM_REQUIRED
    end
    ...
    local function AddMatch(key, childValue, path)
        if type(key) ~= "string" or IsSecret(key) then
            return
        end
