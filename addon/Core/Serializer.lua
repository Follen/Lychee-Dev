...
-- Bounded deterministic serialization. Every cap is a hard number shared with
-- the export pipeline; markers must stay visible when a cap trips.
...
local function SerializeValues(values)
    local lines = {}
    for index = 1, values.n do
        lines[index] = "[" .. index .. "] = " .. Serialize(values[index])
    end
...
-- Coroutine-backed streaming variant: yields at least every 4096 bytes, hands
-- out chunks of at most 44000 bytes, and bounds growth at depth 32 / 10000
-- entries / 16 MB of total output with a visible "... <output limit reached>".
local function CreateStream(value)
...
local function SerializeForExport(value)
    local stream = CreateStream(value)
    local parts = {}
    repeat
