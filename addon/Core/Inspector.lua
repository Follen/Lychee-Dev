...
-- Value trees over live or snapshotted results. The live tree keeps source
-- references so pages can re-inspect; the stored tree is plain data safe for
...
-- Loads one bounded page (200 entries, at most 800 next() scans) into a live
-- table node. Mutation during iteration is surfaced as a marker row.
local function LoadMore(node)
    if ns.Safety and ns.Safety.IsCombatBlocked and ns.Safety.IsCombatBlocked() then
...
-- Live tree over packed return values ({ n = count, [1..n] = values }).
local function CreateLiveTree(values)
...
-- Stored snapshot for history: depth 8 / 4000 nodes / 200 entries per table /
-- 512-byte value text, no live references, visible truncation markers and a
-- `truncated` flag.
local function CreateStoredTree(values)
    if type(values) ~= "table" or type(values.n) ~= "number" or values.n <= 0 then
        return nil
    end
...
ns.Inspector = {
    CreateLiveTree = CreateLiveTree,
    CreateStoredTree = CreateStoredTree,
    LoadMore = LoadMore,
}
