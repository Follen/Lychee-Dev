...
func abandonProbe(ctx context.Context, root, id string) (record journal.WorkRecord, err error) {
	...
	if record.Intent.Kind != "probe" || (record.Stage != "verified" && record.Stage != "abandoning" && record.Stage != "abandoned") {
	...
	if (record.Stage != "verified" && record.Stage != "abandoning") || (record.Status != "running" && record.Status != "unresolved") {
		return record, journal.ErrTransition
	}
	observed, err := verify()
	if err != nil {
