...
var commandContracts = []commandContract{
	...
	{Definition: Definition{Path: "live run", Summary: "Execute one already loaded operation through verified report archival, then stop before acknowledgement: live run <operation-id>", Mutates: true}, positional: "<operation-id>", allowsHome: true},
	...
	{Definition: Definition{Path: "live abandon", Summary: "Explicitly abandon cleanup of a verified probe before ACK; preserve report, retire its exact disk queue entry and release ownership without game input or claiming ACK", Mutates: true}, positional: "<operation-id>", allowsHome: true},
