...
LycheeDevWindow.exportResultButton:Click()
local runEvidence = ns.GetExport(ns.GetExports().order[1])
assert(runEvidence and runEvidence.source.kind == "run_result"
    ...
    "Run did not save its complete result and input as Ticket evidence")
...
assert(rawget(LycheeDevWindow.pageTabs, "performance") == nil
        and rawget(LycheeDevWindow.pages, "performance") == nil
        and rawget(ns, "Performance") == nil and LycheeDevCaptureDock == nil,
    "removed performance tooling is still reachable")
...
local legacyTicket = ns.AddExport("performance_capture", "Legacy performance", "retained legacy payload")
LycheeDevWindow.pageTabs.exports:Click()
local exportPage = LycheeDevWindow.pages.exports
assert(exportPage.ticketBox:GetText() == legacyTicket
        and ns.GetExport(legacyTicket).payload.content == "retained legacy payload",
    "legacy performance evidence was not accessible through saved records")
