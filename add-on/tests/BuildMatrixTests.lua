...
for name, build in pairs(builds) do
    ...
    for index = 1, #files do
        ...
        file:close()
        assert(not files[index]:find("Performance", 1, true), name .. " still loads the performance module")
        assert(not source:find("C_AddOnProfiler", 1, true)
                and not source:find("UpdateAddOnMemoryUsage", 1, true)
                and not source:find('"scriptProfile"', 1, true),
            name .. " still loads performance sampling or a profiling switch: " .. files[index])
    end
