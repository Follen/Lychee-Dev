...
function Invoke-WowdocValidation {
    ...
    $raw = & wowdoc validate --path $Path --source $sourceId --product $Client.product --ref $Client.commit
...
$report = [ordered]@{
    ...
    method = @(
        ...
        'Validate each isolated closure against one locked official wow-ui-source commit.',
